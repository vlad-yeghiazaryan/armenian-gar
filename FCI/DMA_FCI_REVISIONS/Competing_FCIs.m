...
MAFE_final = zeros(t-t0,p,nfore,KK);
MSFE_final = zeros(t-t0,p,nfore,KK);
FE_draw_sq = zeros(t-t0,p,nfore,nsim,KK);
PL_final = zeros(t-t0,p,nfore,KK);
...
%     end
        ...
        if irep>t0
            ...
            for isim = 1:nsim           
                ...
                for ifore = 1:nfore
                    ...
                    y_f_draw(ifore,:,isim,nmod) = MEAN_FOR(1:p,:) + chol(VAR_FOR(1:p,1:p))'*randn(p,1);
                end
            end           
        end        
    end
    ...
    if irep>t0
        for nmod = 1:KK
            mean_fore = mean(y_f_draw(:,:,:,nmod),3);
            for ii = 1:nfore           
                FE_draw_sq(irep-t0,:,ii,:,nmod) = ( repmat(Yraw_f(irep+ii,:)',1,nsim) - squeeze(y_f_draw(ii,:,:,nmod)) ).^2;
                MSFE_final(irep-t0,:,ii,nmod) = ( Yraw_f(irep+ii,:) - mean_fore(ii,:) ).^2;           
                MAFE_final(irep-t0,:,ii,nmod) = abs( Yraw_f(irep+ii,:) - mean_fore(ii,:) );
                for jj = 1:p
                    PL_final(irep-t0,jj,ii,nmod) = ksdensity(squeeze(y_f_draw(ii,jj,:,nmod)),Yraw_f(irep+ii,jj));
                end
            end
        end
    end
