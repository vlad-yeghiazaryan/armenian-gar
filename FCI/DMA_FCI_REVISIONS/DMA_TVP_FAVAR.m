...
var_no_dma = 1:3;    % Choose variables always included in each model (to help identify factors).
                     % Single model case is var_no_dma = 1:18 (all variables included, no dma).                  
...
for irep = t0:t-1
    ...
    parfor nmod = 1:KK
        ...
        if irep>t0 && irep<t
            ...
            [y_f,PL_1] = DMAFCI_fore(Yraw_f(irep+1,:),beta_t(:,:,irep),factors,Q_t(:,:,irep),p,nfore);
            y_fore(:,:,nmod) = y_f;
            PL(:,nmod) = PL_1;
        end        
    end
