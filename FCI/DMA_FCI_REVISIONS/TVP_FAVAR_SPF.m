% TVP_FAVAR_SPF - Time-varying parameters factor-augmented VAR using an adaptive Kalman filter
...
% l_1 = 1;       % Decay factor for measurement error variance
% l_2 = 1;       % Decay factor for factor error variance
% l_3 = 1;       % Forgetting factor for loadings error variance
% l_4 = 1;       % Forgetting factor for VAR coefficients error variance
...
load xdata.dat;
% load SPF forecast used to purge factors
load spf.dat;
...
lambda_0.mean = zeros(n,rr);
lambda_0.var = 4*eye(rr);
...
for irep = t0:t-1
    ...
    [beta_t,beta_new,Sb_t,lambda_t,Sl_t,V_t,Q_t] = KFS_parameters_SPF(YX,YF,YFF,l,nfac,nlag,k,m,p,n,r,rr,irep,lambda_0,beta_0,V_0,Q_0);
    
    % ====| STEP 2: Update Factors Conditional on TV-Parameters   
    [factor_new,Sf_t_new] = KFS_factors_SPF(YX,Y_SPF,lambda_t,beta_t,V_t,Q_t,nlag,k,r,p,q,irep,factor_0);
