% TVP_FAVAR_TS - Time-varying parameters factor-augmented VAR using an adaptive Kalman filter
...
%-----------------------------------------------------------------------------------------
%  - This code estimates an FCI with all available financial variables
...
X_tr = standardize_miss(xdata(1:tau,:)) + 1e-40; X_tr(isnan(X_tr)) = 0; Y_tr = [macro_p(1:tau,tau+1), macro_ruc(1:tau,tau+1), macro_routput(1:tau,tau+1)];  % Training data
...
xdata = xdata(tau+1:end,:);
t = size(xdata,1);                % new sample size (t = t_full - tau)
...
factor_0.var = 1*eye(k);      factor_0.var(1:r,1:r) = sigmaf;
...
V_0 = 0.01*eye(q) + sigma2; V_0(1:p,1:p) = 0;
Q_0 = sigmaf;
...
Yraw_f = [[macro_p(tau+1:end,end), macro_ruc(tau+1:end,end), macro_routput(tau+1:end,end)] ; NaN(nfore,p)];
...
for irep = t0:t-1
    ...
    if irep<t0
        ...
        Y = [macro_p(tau+1:tau+t0,1), macro_ruc(tau+1:tau+t0,tau+1), macro_routput(tau+1:tau+t0,tau+1)];
    elseif irep>=t0      
        X_st = standardize_miss(xdata(1:irep,:));% + 1e-20;
        X_st(isnan(X_st)) = 0;
        %[Y,Ymeans,Ystds] = standardize2(ydata(1:irep,:));   
        Y = [macro_p(tau+1:tau+irep,irep+1-t0), macro_ruc(tau+1:tau+irep,irep+1-t0), macro_routput(tau+1:tau+irep,irep+1-t0)];
    end
