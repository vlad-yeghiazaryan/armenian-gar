function [beta_t,beta_new,Sb_t,lambda_t,Sl_t,V_t,Q_t] = KFS_parameters(YX,FPC,l,nfac,nlag,y_true,k,m,p,q,r,t,lambda_0,beta_0,V_0,Q_0)
...
for irep = 1:t  
    ...
    if l_1 == 1
        A_t = e_t(p+1:end,1:irep)*e_t(p+1:end,1:irep)';
        if irep < 10
            V_t(:,:,irep) = diag(diag(V_0));
        else
            V_t(p+1:end,p+1:end,irep) = (1./irep)*diag(diag(A_t)) + 1e-10*eye(q-p);
        end        
    else
